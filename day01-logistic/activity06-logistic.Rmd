...
1. Is this an observational study or an experiment?
  ...
  This is an experimental data, as suggested in the description before. Variables have been manufactured.
...
2. The variable of interest is `received_callback`.
  ...
  It is a binary categorial data type where 0 suggests false and 1 suggests true for getting call back for the job. 
...
ggplot(resume, aes(x = factor(received_callback))) +
  geom_bar(stat = "count", fill = "#66c2a5", width = 0.7, color = "white") +
  labs(
...
```{r}
percentage_data <- resume %>%
  group_by(received_callback) %>%
  summarize(count = n()) %>%
  mutate(percentage = count / sum(count) * 100)

# Create a pie chart with percentage labels
ggplot(percentage_data, aes(x = "", y = percentage, fill = factor(received_callback))) +
...
Only a small number of people are getting callbacks. 
...
```{r}
probability_callback <- mean(resume$received_callback == 1)
...
# Calculate the odds of receiving a callback
odds_callback <- 1- ( probability_callback / (1 - probability_callback))

# Print the result
cat("Odds of receiving a callback:", odds_callback, "\n")
```
...
*It creates a subset of observations in Chicago and renames the race and gender features.*
...
* Numbers of females more than males*
...
resume_mod2 <- logistic_spec %>%
  fit(received_callback ~ race, data = resume_subet, family = "binomial")
...
mult_log_aug <- augment(resume_mod, new_data = resume, type.predict = "response", 
